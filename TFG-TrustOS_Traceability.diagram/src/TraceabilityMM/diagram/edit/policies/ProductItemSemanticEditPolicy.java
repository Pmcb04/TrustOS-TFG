...
public class ProductItemSemanticEditPolicy
	...
	*/
	public ProductItemSemanticEditPolicy() {
		super(TraceabilityMM.diagram.providers.TraceabilityMMElementTypes.Product_2002);
	}
	...
	*/
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (TraceabilityMM.diagram.part.TraceabilityMMVisualIDRegistry.getVisualID(
					incomingLink) == TraceabilityMM.diagram.edit.parts.TransactionNext_productEditPart.VISUAL_ID) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (TraceabilityMM.diagram.part.TraceabilityMMVisualIDRegistry.getVisualID(
					outgoingLink) == TraceabilityMM.diagram.edit.parts.ProductNext_transactionEditPart.VISUAL_ID) {
				DestroyReferenceRequest r = new DestroyReferenceRequest(outgoingLink.getSource().getElement(), null,
						outgoingLink.getTarget().getElement(), false);
				cmd.add(new DestroyReferenceCommand(r));
				cmd.add(new DeleteCommand(getEditingDomain(), outgoingLink));
	...
	protected Command getStartCreateRelationshipCommand(CreateRelationshipRequest req) {
		...
		if (TraceabilityMM.diagram.providers.TraceabilityMMElementTypes.ProductProductconf_4003 == req
				.getElementType()) {
